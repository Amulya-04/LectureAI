...
RAG_PROMPT = PromptTemplate(
    ...
    template="""You are a personalized AI lecture tutor. You are tutoring a **{user_level}** level student.

Level guidance:
- Beginner: Use simple everyday language, relatable analogies, avoid jargon. Break things down step by step.
...
Use ONLY the following lecture transcript excerpts to answer the student's question.
...
Answer clearly, referencing specific parts of the lecture when relevant. 
Tailor your vocabulary, depth and examples to a **{user_level}** student:""",
)
...
class RAGPipeline:
    ...
    def __init__(self, llm: BaseLanguageModel, user_level: str = "Beginner"):
        self.llm = llm
    ...
    def build_index(self, transcript: str) -> None:
        ...
        # Binds user_level so it flows into every prompt invocation
        self._rebuild_chain()

    def _rebuild_chain(self):
        """Rebuild the LCEL chain with the current user_level."""
        ...
        self.chain = (
            {
                ...
                "user_level": lambda _: self.user_level,
            }
            | RAG_PROMPT
            | self.llm
            | StrOutputParser()
    ...
    def set_user_level(self, level: str):
        """Dynamically update user level and rebuild chain accordingly."""
